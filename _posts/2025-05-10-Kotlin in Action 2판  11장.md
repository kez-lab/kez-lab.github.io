...
- **가능한 작업**  
  ...
  - 함수 호출 시 타입 인자로 사용  
- **불가능한 작업**  
  1. `T()` 같은 생성자 호출로 인스턴스 생성  
  2. `T.Companion` 같은 companion 객체 접근  
  3. `reified` 없는 일반 함수에 `reified` 타입 파라미터 지정  
  4. non-inline 함수에서 `reified` 사용  
...
- **타입 vs 클래스**  
  - 클래스(`String`) 하나로도 `String` vs `String?` 두 가지 타입이 존재  
- **하위 타입(subtype)**  
  - B가 A가 필요한 곳에 문제 없이 쓰이면 B는 A의 하위 타입  
  - 예: `Int` <: `Number`, `String` <: `Any`  
- **제네릭 타입에서의 하위 타입**  
  - 기본적으로 `MutableList<A>`와 `MutableList<B>`는 A와 B의 관계와 무관하게 별개의 타입(`invariant`)  

---

### 11.3.3 공변성(Covariance)  
- **정의**: A <: B이면 `Container<A>` <: `Container<B>` 관계 유지  
- **Kotlin 선언**: 타입 파라미터 앞에 `out` 키워드  
  ```kotlin
...
- **정의**: A <: B이면 `Consumer<B>` <: `Consumer<A>` 관계 뒤집힘  
- **Kotlin 선언**: 타입 파라미터 앞에 `in` 키워드  
  ```kotlin
...
- **Java 와일드카드 대응**  
  - `? extends T` ↔ `MutableList<out T>`  
  - `? super T`   ↔ `MutableList<in T>`  
- **예시: 데이터 복사 함수**  
  ```kotlin
  // 복사 대상 source는 읽기 전용 프로젝션(out)
  fun <T> copyData(
  ...
  // 또는 destination을 쓰기 전용 프로젝션(in)으로
  fun <T> copyData2(
...
### 11.3.6 스타 프로젝션(Star Projection)  
- **문법**: `List<*>`, `MutableList<*>`  
- **의미**: “어떤 타입 T인지 모른다”  
  - 꺼낼 때는 `Any?` 타입으로 안전하게 핸들링  
  - 추가는 금지 (`out` 위치로만 취급)  
- **예시**  
  ```kotlin
  fun printFirst(list: List<*>) {
    if (list.isNotEmpty()) {
      println(list.first()) // Any?
    }
  }
  ```
...
### 11.3.7 타입 별명(Type Alias)  
- **문법**: `typealias Alias = 원래타입`  
- **용도**: 복잡한 제네릭/함수 타입에 이름을 붙여 가독성 향상  
- **예시**  
  ```kotlin
  typealias NameCombiner = (String, String, String, String) -> String

  val authors: NameCombiner = { a, b, c, d -> "$a et al." }
  fun combine(c: NameCombiner): String = c("A","B","C","D")
  ```
