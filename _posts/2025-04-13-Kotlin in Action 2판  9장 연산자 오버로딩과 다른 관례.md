...
코틀린에는 **사용자가 정의한 함수**를 언어 기능이 호출하는 **관례(convention)** 가 있음. 예를 들어, `for...in` 루프는 `iterator`를 호출하고, `+` 연산자는 `plus` 함수를 호출함. 이처럼 코틀린은 특정 클래스나 인터페이스에 강하게 묶여 있지 않고, **특정 이름을 가진 함수의 존재만으로 언어 기능을 사용할 수 있음**.
...
자바와 달리 코틀린은 이러한 기능을 **특정 클래스가 아닌 함수 이름에 기반하여 연결**함.  
이는 자바의 기존 클래스를 확장하거나, 새로운 기능을 기존 클래스에 부여할 수 있게 해줌.  
이러한 설계는 **확장 함수**를 활용한 유연한 방식으로, 기존 자바 클래스를 쉽게 활용할 수 있도록 해줌.
