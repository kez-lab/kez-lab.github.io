...
코루틴은 일시 중단 가능한 계산의 인스턴스임.  
다른 코루틴들과 동시에(혹은 심지어 병렬로) 실행될 수 있는 코드 블록임.  
스레드와 비슷하지만, 코루틴은 함수 실행을 일시 중단하는 데 필요한 메커니즘을 포함함.  
코루틴을 생성할 때 코루틴 빌더 함수 중 하나를 사용함.
...
일반 블로킹 코드를 일시 중단 함수의 세계로 연결하려면 runBlocking 코루틴 빌더 함수에 코루틴 본문을 전달함.  
이렇게 하면 새 코루틴을 생성하고 실행하며, 해당 코루틴이 완료될 때까지 현재 스레드를 블록시킴.  
...
```
runBlocking을 사용할 때는 하나의 스레드를 블로킹함.  
이 코루틴 안에서는 추가 자식 코루틴을 얼마든지 시작할 수 있고,  
...
launch 함수는 새로운 자식 코루틴을 시작하는 데 사용함.  
일반적으로 "fire-and-forget" 시나리오에 사용되며, 어떤 코드를 실행하지만 결과 값을 기다리지 않을 때 적합함.
...
- launch는 Job 타입 객체를 반환하며, 코루틴 실행 제어에 사용 가능(예: 취소).

---

#### 14.6.3 대기 가능한 연산: async 빌더

async 빌더는 비동기 계산을 수행할 때 사용함.  
launch와 마찬가지로 async에도 실행할 코드를 코루틴으로 전달할 수 있음.  
하지만 async의 반환 타입은 launch와 달리 Deferred<T>임.  
...
코루틴의 디스패처(Dispatcher)는 코루틴이 어떤 스레드나 스레드 풀에서 실행될지 결정함.  
부모 코루틴으로부터 디스패처를 상속받기 때문에 명시적으로 지정하지 않아도 되지만,  
필요하다면 다음과 같이 여러 디스패처를 선택할 수 있음.
...
```
- 백그라운드 작업 후, 결과를 UI 스레드에서 안전하게 처리하는 전형적 패턴임.
- withContext는 언제든 다른 디스패처로 전환해 실행 가능.
...
- CoroutineContext는 여러 요소로 이루어진 집합임.
- 이 요소 중 하나가 코루틴의 실행 스레드를 결정하는 Dispatcher임.
- CoroutineContext에는 보통 코루틴의 생명주기와 취소를 관리하는 Job 객체,  
  CoroutineName, CoroutineExceptionHandler 같은 추가 메타데이터도 포함 가능함.

코루틴의 현재 CoroutineContext는 언제든지 일시 중단 함수 안에서 coroutineContext 속성으로 확인 가능함.
...
- CoroutineContext의 중요성, Job·ExceptionHandler 등 추가 정보는 이후 15장에서 더 자세히 다룸.
...
- Default/IO는 다중 스레드 디스패처이므로 데이터 경쟁에 주의.
